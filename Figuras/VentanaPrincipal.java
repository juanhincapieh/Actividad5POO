...
public class VentanaPrincipal extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evento) {
        if (evento.getSource() == esfera) { // Si se pulsa el botón esfera
            VentanaEsfera esf = new VentanaEsfera(); //Crea la ventana de la esfera
            esf.setVisible(true); //Establece que se visualice la ventana de la esfera
        }
        if (evento.getSource() == cilindro) { //Si se pulsa el botón cilindro
            VentanaCilindro cili = new VentanaCilindro(); //Crea la ventana del cilindro
            cili.setVisible(true); //Establece que se visualice la ventana del cilindro
        }
        if (evento.getSource() == piramide) { //Si se pulsa el botón pirámide
            VentanaPiramide pir = new VentanaPiramide(); //Crea la ventana de la pirámide
            pir.setVisible(true); //Establece que se visualice la ventana de la pirámide
        }
    }
